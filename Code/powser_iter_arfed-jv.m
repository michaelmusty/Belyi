    ...
    for dim_cnt := 1 to dim do
      ...
      repeat
        ...
        if true then // to get Magma timing
          ...
          qis := [];
          for j := 1 to nv do
            qis cat:= Eltseq(Vector(qi[qiind+1..qiind+NNs[j]])*Wps[j]);
            qiind +:= NNs[j];
          end for;
          qi := qis;
          if Federalize then
          ...
          qis := [];
          for j := 1 to nv do
            qis cat:= Eltseq(Vector(qi[(j-1)*Q+1..j*Q])*Js[j]);
          end for;
          qi := Vector(qis)/Q;
          ...
          */
          ...
          ysol := [CC | 0 : j in [1..i-2]] cat [1];
          for j := i-2 to 1 by -1 do
            ysol[j] := -(&+[ysol[kk]*h[kk,j+1] : kk in [j+1..i-1]] - ysol[j+1]);
            for kk := j+1 to i-1 do
              ysol[kk] *:= h[j,j+1];
            end for;
          end for;
          ysol := [c/ysol[1] : c in ysol];
          /*
          print ysol0;
          H0 := ColumnSubmatrix(Matrix(h),i-1);
          kersiz := Matrix(CC,1,i-1,ysol0)*(H0-1);
          print kersiz;
           
          ysol, sings := NumericalKernel_old(H0-1 : Epsilon := eps_thresh);
          minsingprev := minsing;
          minsing := Min([Abs(s) : s in sings]);
          vprintf Shimura : "min %o (of %o)... \n", RealField(6)!minsing, #ysol;
          if #ysol ge 1 then
            ey := Eltseq(ysol[1]/ysol[1][1]);
            print ey;
            yFound := true;
          ...
          */
          minsing := Abs(&+[ysol[kk]*h[kk,1] : kk in [1..i-1]]-ysol[1]);
        ...
        if i gt 50 then
          h := [];
          Q0 := RowSubmatrix(Matrix(q),i-1);
          q0 := Vector(Matrix(CC,1,i-1,ysol)*Q0);
          q0 /:= Sqrt(Norm(q0));
          q := [q0];
          i := 1;
          vprint Shimura: "... Restarting!\n\n";
        end if;

      until minsing lt eps_thresh;
      vprintf Shimura : "escaped with cutoff %o...\n", RealField(6)!minsing;

      Q0 := RowSubmatrix(Matrix(q),i-1);
      Append(~x, Matrix(CC,1,i-1,ysol)*Q0);
